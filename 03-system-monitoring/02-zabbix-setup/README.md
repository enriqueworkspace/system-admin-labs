...
wget https://repo.zabbix.com/zabbix/7.0/ubuntu/pool/main/z/zabbix-release/zabbix-release\_7.0-2+ubuntu24.04\_all.deb

sudo dpkg -i zabbix-release\_7.0-2+ubuntu24.04\_all.deb  # Pressed Y to overwrite previous repo
...
\- Set root password: \[MYSQL\_ROOT\_PASSWORD]
...
CREATE USER 'zabbix'@'localhost' IDENTIFIED WITH mysql\_native\_password BY '\[ZABBIX\_DB\_PASSWORD]';
...
sudo mysql -u root -p  # Set GLOBAL log\_bin\_trust\_function\_creators = 1
...
DBPassword=\[ZABBIX\_DB\_PASSWORD]
...
sudo nano /etc/zabbix/apache.conf  # Added php\_value date.timezone America/New\_York
...
\- Completed wizard; logged in as Admin/zabbix (changed to \[ADMIN\_PASSWORD]).
...
sudo nano /etc/zabbix/zabbix\_agent2.conf
...
Invoke-WebRequest -Uri "https://cdn.zabbix.com/zabbix/binaries/stable/7.0/7.0.19/zabbix\_agent2-7.0.19-windows-amd64-openssl.msi" -OutFile "$tempPath\\zabbix\_agent2.msi"

msiexec /i "$tempPath\\zabbix\_agent2.msi" /qn SERVER="192.168.0.179" SERVERACTIVE="192.168.0.179" HOSTNAME="windows-host" STARTSERVICE=1
...
\- Key: service\_state\[Windows Update,wuauserv]
...
&nbsp;  - Operations: Send message to Admin (configured email media in Administration > Users > Admin > Media: \[SMTP\_SERVER], \[EMAIL]).
